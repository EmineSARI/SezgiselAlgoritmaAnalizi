...
CostFunction=@(m) ClusteringCost(m, X);     % Cost Function Küme merkezsleri rastgele atýlan deðerler

VarSize=[k size(X,2)];  % Decision Variables Matrix Size/Karar Deðiþkenleri Matris Boyutu

nVar=prod(VarSize);     % Number of Decision Variables

VarMin= repmat(min(X),k,1);      %  küme merkezi rasgele sýnýrlarýný atýyor 
VarMax= repmat(max(X),k,1);      %Deðiþkenlerin Üst Sýnýrý

%% GA Parameters

MaxIt=100;      % Maksimum Yineleme Sayýsý

nPop=30;        % Populasyon boyutu

pc=0.8;                 % genetik deðiþim yüzdesi
